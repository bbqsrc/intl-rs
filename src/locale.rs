...
#[cfg(windows)]
mod platform {
    use winapi::{um::winnls, ctypes::c_int};
    ...
    pub fn user_default_locale_name() -> Result<String, std::io::Error> {
...
impl Default for Locale {
    fn default() -> Locale {
        if let Ok(v) = platform::user_default_locale_name() {
            return Locale { tag: v.parse().unwrap() };
        }

        Locale { tag: "und".parse().unwrap() }
    }
}
